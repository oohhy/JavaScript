...
- 관계 연산자
  - 키만 가지고 판단
  ```js
  10 in [10, 20, 30]; // false(key만 보기 때문에)
  1 in [10, 20, 30]; // true
  1 in "hello"; // error
...
- 참조타입(reference types) : object(object, array, map, set), function (원본값이 변함)
...
- Number(숫자)
  ...
  - Math
    - Math.PI
    - Math.max() Infinity와 비교
    - Math.min()  
      let test = [10, 20, 30]
      Math.min(test) -x
      Math.min(...test) -o
    - Math.floor()
      Math.floor(Math.random() \* 11): 0~10 랜덤으로 뽑기
    - Math.round()
    - Math.random()
    - Math.abs()
    - Math.sqrt()
...
- String(문자열)
  ...
  - 메서드 :
    ...
    - str.includes()
    - str.slice() v
    - str.splice() v 배열에 있는 메서드. str에는 없음
    - str.split() v '!'.repeat(99).split('!')
    - str.substring()
    - str.substr()
    - str.toLowerCase()
    - str.toUpperCase()
    ...
    "hello".repeat(100);
    "0".repeat(100);
    "5".repeat(100).split("").map(Number); //Number는 콜백함수
    ```
...
- Array(배열) : object
  ...
  - 메서드
    - length
    - forEach : 단순 순회해서 요소 출력!
    - map : return값을 새로운 배열에 푸시!
    - filter : return이 true인 요소를 새로운 배열에 푸시!
    - push / pop - mutable(원본 수정)
    - slice - immutable(원본 유지)
    - splice - mutable
    - reduce - immutable
    - join 합치기
    - indexOf
...
- Map : object
  ...
  map.set("three", 300);
  map.set("four", [10, 20]);
  map.set(5, [100, 200]); // key값이 숫자일때 마침표 호출 불가능
  map.set([1, 2], [100, 200]); // key에 배열 가능하지만 호출은 불가능
  map.get(5);
...
- Set : object
  ...
  let a = new Set([1, 2, 3]);
  let b = new Set([3, 4, 5]);
  const 합집합 = new Set([...a, ...b]); //new Set안하면 [1, 2, 3, 3, 4, 5]
...
break - 반복문 1겹만 탈출
continue - 거기서 멈추고 다음 반복 순서로 넘어감
